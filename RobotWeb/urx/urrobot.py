...
from urx import urrtmon, ursecmon
...
class URRobot(object):
    ...
    def get_forward_kin(self,qpose,ppose):
        qpose = [round(i, self.max_float_length) for i in qpose]
        ppose = [round(i, self.max_float_length) for i in ppose]
        message = "get_forward_kin(%s,p%s)" % (qpose,ppose)
        self.send_program(message)
